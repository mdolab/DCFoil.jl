...
\mbf{0}
\eeq
which we do through the Gauss-Seidel iteration in OpenMDAO. % following the pseudocode of \citet{Kenway2014a}.
...
\end{equation}
and the moment consistency with respect to the flow collocation point (where the moment on the flow side is zero by definition):   % NOTE: I think technically this is only valid if collocation point = center of pressure, which is not always the case
\begin{equation}
	\mb{M}_{1,i} + \mb{M}_{2,i} + (-\mb{r}_{1, i} \times \mb{F}_{1,i}) + (-\mb{r}_{2, i} \times \mb{F}_{2,i}) = 0
\end{equation}
% TODO: check consistency of moment?
...
% \eeq
% % This equation is written like this because it more accurately represents how it would be coded up for 2 disciplines
