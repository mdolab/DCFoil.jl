...
\right)
\ee
which you can use after you solve for $\phi$ to determine wave elevation~\cite{Hess1967a}.
...
\be
\label{eqn:LSFSBC}
\boxed{\Uinf^2 \ppt{\phi}{x} + g\pp{\phi}{z}}
= - \left( \ppt{\phi}{t}
...
\right)
= 0
\quad \tn{(Free surface Dirichlet BC)}
\ee
...
\een
so we can actually evaluate stuff like Equation~\eqref{eqn:BCs}.
TODO: I think
...
+ \frac{\mu}{4\pi}
\frac{x}{\left[x^2 + y^2 + \left(z-1\right)^2 \right]^{3/2}}
\ee
...
\frac{\sigma(\mbf{x}_s)}{\left| \mbf{x}_c - \mbf{x}_s\right|}
+ \frac{x}{4\pi \left| \mbf{x} - \mbf{x}_0\right|^{3/2}}
\right)
\tn{d}\Omega
\ee
determined from numerical integration. TODO: GGGGGGGGGGGGGGGGGGGGGGGGG code this up
Check \citet{Bandyk2009} thesis Appendix B for how to do normal velocity...

Now if we do a dipole moving in steady forward speed, the linear system is
\be
\Uinf^2  \iint_{\Omega}\ppt{   }{x}
\left(
\sigma(\mbf{x}_s) \frac{1}{\left| \mbf{x}_c - \mbf{x}_s\right|} \tn{d}\Omega
...
% We will go through a steady dipole problem with forward speed, we have a fairly small linear system to solve.
...
The partial derivative matrices in Equation~\eqref{eqn:Adjoint} are computed through finite differences.
