...
\subsubsection{Free surface effect}

We compute the nonlinear free surface effect using the method of de-singularization from \citet{Cao1991}.
The \emph{indirect method} solves for the strengths of the singularities $\sigma$.
The velocity potential (field function) is some integral of a source distribution $\sigma(\mbf{x}_s)$ over the surface $\Omega$ outside the problem domain
\be
...
\be
&\iint_{\Omega}\sigma(\mbf{x}_s)\frac{1}{\left| \mbf{x}_c - \mbf{x}_s \right| } \tn{d} \Omega & = \phi_0(\mbf{x}_c) \quad \tn{(Dirichlet)}
\\
& \iint_{\Omega} \sigma(\mbf{x}_s) \pp{}{n}\left(\frac{1}{\left| \mbf{x}_c - \mbf{x}_s\right|}\right)\tn{d} \Omega & = \chi(\mbf{x}_c) \quad \tn{(Neumann)}
\ee

The nonlinear dynamic and kinematic boundary conditions for the free surface (derived from the unsteady Bernoulli equation) are
\be
\label{eqn:FSBC}
& \DD{\phi}{t} & = - {z}_f + \half \nabla \phi \cdot \nabla \phi \quad \tn{(dynamic)}
\\
...
\begin{algorithm}
    ...
    \caption{\label{alg:FSBC}
        Computation of hydrodynamic loads with a free surface boundary condition
    }
    \KwData{Grid points (paneleing of geometry) and flow speed}
    ...
    Form your Dirichlet and Neumann conditions\;
    Impose the nonlinear dynamic and kinematic free surface boundary conditions at $z=0$ (Equation \eqref{eqn:FSBC}).
    These need to be integrated in time to steady state.
    \;
\end{algorithm}
%------------------------------------------------------------------------------
\clearpage
...
\be
&\mbf{K_s} \mbf{u} & = \mbf{f}_{\tn{hydro}}
\\
&\left(\mbf{K_s} - \mbf{K_f}\right)\mbf{u} & = \mbf{0}
\ee
