...
% --- Math ---
\newcommand{\diff}{\mathop{}\!\mathrm{d}} % total derivative
...
% \newcommand{\det}{\textrm{det}} % command exists from amsmath
% --- Equations ---
...
\begin{algorithm}[htb!]
    ...
    \quad \quad Code to compute $\bar{x}= \bar{y} \dd{y}{x}$ \tcp*{Ex. $\bar{x} =\bar{y} \cos(x)  $ for $f(x)=\sin(x)$}
